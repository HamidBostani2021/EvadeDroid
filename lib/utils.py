...
def my_lock(mode):
    lock = os.path.join(config['ice_box'],'lockfile')
    if mode == "create":
...
def run_java_component(jar, args, timeout=None):
    ...
    cmd = [config['java_sdk'] + 'java', '-jar', jar, *args]
...
def sanitize_url_feature_name(feature):
    return feature.replace(".", "_").replace("/", "£").replace(":", "^").replace("?", "@").replace(">", "(").replace(";", ")").replace(" ","");
...
def perform_logging_for_attack(message):
    message = str(message)
    log_path = os.path.join(config["project_root"] , "log_attack.txt")
    if os.path.exists(log_path) == False:
         with open(log_path, 'w') as f:
             f.write(message + '\n')             
    else:
