...
public class Soot_utlilty {
    ...
    //get all names of the directories, which corresponds to the APKs
    ...
    //get all classnames from all the jimple files
    ...
    public void initSoot(String apkPath, ArrayList<File> dexPath, File[] directories,String output){
        ...
        for(File f : dexPath){
            //Hamid paths.add(f.getAbsolutePath());
        	paths.add(f.getPath());
        }
    ...
    public My_slice get_random_slice(ArrayList<My_slice> slices){
        int randomNum = ThreadLocalRandom.current().nextInt(0, slices.size());
        My_slice slice =  slices.get(randomNum);
        ArrayList<Unit> units = new ArrayList<>(slice.getMy_units());
        ArrayList<Local> locals = new ArrayList<>(slice.getMy_locals());
        My_slice slice_t = new My_slice(locals, units, slice.getName(), slice.getFeature());
        return slice_t;
    }
    ...
    public ArrayList<SootClass> count_classes() {
        ArrayList<String> tmp_all_classes = new ArrayList<>();
        ArrayList<SootClass> to_return = new ArrayList<>();
        for(SootClass c : Scene.v().getClasses()){
            if(this.isExcludedLibrary(c.getName()) || c.isPhantom() || c.isPhantomClass() ){
    ...
    public Map<String,Integer> get_opaque_scores(String opaque) {   	
        ...
        for(String s : predicates){
            ...
            try (FileInputStream is = new FileInputStream(path)) {
            	...
            	try {            		
                    //Walker w = new Walker(SootResolver.v().resolveClass("com.shoutcast.stm.fmhuayraquimbal.chatActivity", SootClass.DANGLING), null);
            		Options.v().set_whole_program(true);
            		//Scene.v().addBasicClass("com.shoutcast.stm.fmhuayraquimbal.chatActivity",SootClass.DANGLING);
            		//Walker w = new Walker(SootResolver.v().resolveClass("com.shoutcast.stm.fmhuayraquimbal.chatActivity", SootClass.HIERARCHY), null);
            		System.out.println(SootClass.DANGLING);
            		System.out.println(SootClass.HIERARCHY);
            		System.out.println(SootClass.SIGNATURES);
            		Walker w = new Walker(new SootClass("com.shoutcast.stm.fmhuayraquimbal.chatActivity",SootClass.HIERARCHY), new SootResolver(null));   
            		tree.apply(w);
                    SootClass sc = w.getSootClass();
                    int cc =ju.calculate_cyclomatic_complexity(sc);
                    to_ret.put(s,cc);
