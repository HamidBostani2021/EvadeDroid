...
#This package cannot install on Sandor machine
#import ssdeep
...
#CC = ''.join(map(chr, range(0, 32) + range(127, 160)))
CC = ''.join(map(chr, range(0, 32)))
sha = None
...
def hash(fileSystemPosition):
    try:
        ssdeepValue = None#ssdeep.hash_from_file(fileSystemPosition)
        return ssdeepValue
...
def copyIcon(sampleFile, unpackLocation, workingDir):
    ...
    manifest = subprocess.Popen([settings.AAPT, 'dump', 'badging', sampleFile],
                                ...
                                #encoding='utf8')#Hamid add this argument: https://stackoverflow.com/questions/44989808/subprocess-typeerror-a-bytes-like-object-is-required-not-str
...
def unpackSample(tmpDir, sampleFile):
    ...
    #unpackLocation = tmpDir + sampleFile.split('\\')[-1].split('.')[0]
    unpackLocation = tmpDir + os.path.basename(sampleFile).replace(".apk","")
    #print("feature extraction - unpackLocation: " + unpackLocation)
    if not os.path.exists(unpackLocation):
        #print("make unpackLocation")
        os.mkdir(unpackLocation)    
    #print("unzip " + "-o -q -d " + unpackLocation + " " + sampleFile)
    os.system("unzip " + "-o -q -d " + unpackLocation + " " + sampleFile) #https://www.somacon.com/p161.php
