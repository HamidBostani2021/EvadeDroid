...
public class Soot_utlilty {
    ...
    //Hamid
    public ArrayList<SootMethod> find_method_for_feature_api_call(SootClass searched, String feature) {
    ...
    public ArrayList<SootMethod> find_method_for_feature_intent(SootClass searched, String feature) {
        ...
        if(searched.getFields().size()>0){
            for(SootField f : searched.getFields()){                
                if(f.toString().contains(feature)){
                    correspondence = feature+"|"+"android.content.Intent";
                    break;
                }                
