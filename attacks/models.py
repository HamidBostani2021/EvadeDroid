...
class SVM(SVMModel):
    ...
    def generate_model_name(self):
        ...
        elif self.malware_detector == "Drebin":
            model_name = 'svm'
        elif self.malware_detector == "Drebin-Robust":
            model_name = 'svm-robust'
...
def create_roc_curve():   
    roc_curve = dict()
    import json
    path_roc_curve = "C:/GitLab/end-to-end_black-box_evasion_attack/Results/roc_curve.json"
    roc_curve_exists = False
    if os.path.exists(path_roc_curve) == True:
        with open(path_roc_curve,'r') as file:
            roc_curve = json.load(file)
        roc_curve_exists = True
            
    print("os.path.exists(path_roc_curve):",os.path.exists(path_roc_curve))
    ...
    if roc_curve_exists == True:
        roc_info = [np.array(roc_curve['drebin'][0]),np.array(roc_curve['drebin'][1]),np.array(roc_curve['drebin'][2])]    
        mean_tpr_drebin,mean_fpr_drebin,std_tpr_drebin = plot_roc_curve_zoom(model_main,"Drebin",roc_info)
    else:
        mean_tpr_drebin,mean_fpr_drebin,std_tpr_drebin = plot_roc_curve_zoom(model_main,"Drebin")
    if roc_curve_exists == False:
        roc_curve['drebin'] = [list(mean_tpr_drebin),list(mean_fpr_drebin),list(std_tpr_drebin)]
        with open(path_roc_curve,'w') as file:
            json.dump(roc_curve,file)
    
    if roc_curve_exists == True:
        roc_info = [np.array(roc_curve['secsvm'][0]),np.array(roc_curve['secsvm'][1]),np.array(roc_curve['secsvm'][2])]
        mean_tpr_secsvm,mean_fpr_secsvm,std_tpr_secsvm = plot_roc_curve_zoom(model_main,"Sec-SVM",roc_info)
    else:
        mean_tpr_secsvm,mean_fpr_secsvm,std_tpr_secsvm = plot_roc_curve_zoom(model_main,"Sec-SVM")
    if roc_curve_exists == False:
        roc_curve['secsvm'] = [list(mean_tpr_secsvm),list(mean_fpr_secsvm),list(std_tpr_secsvm)]    
        with open(path_roc_curve,'w') as file:
            json.dump(roc_curve,file)
    
    if roc_curve_exists == True:
        roc_info = [np.array(roc_curve['adema'][0]),np.array(roc_curve['adema'][1]),np.array(roc_curve['adema'][2])]
        mean_tpr_adema,mean_fpr_adema,std_tpr_adema = plot_roc_curve_zoom(model_main,"AdversarialDeepEnsembleMax",roc_info) 
    else:
        mean_tpr_adema,mean_fpr_adema,std_tpr_adema = plot_roc_curve_zoom(model_main,"AdversarialDeepEnsembleMax") 
    if roc_curve_exists == False:
        roc_curve['adema'] = [list(mean_tpr_adema),list(mean_fpr_adema),list(std_tpr_adema)]  
        with open(path_roc_curve,'w') as file:
            json.dump(roc_curve,file)
    ...
    roc_info = [np.array(roc_curve['mamadroid'][0]),np.array(roc_curve['mamadroid'][1]),np.array(roc_curve['mamadroid'][2])]
    mean_tpr_mamadroid,mean_fpr_mamadroid,std_tpr_mamadroid = plot_roc_curve_zoom(model_main,"MaMaDroid",roc_info)
    if roc_curve_exists == False:
        roc_curve['mamadroid'] = [list(mean_tpr_mamadroid),list(mean_fpr_mamadroid),list(std_tpr_mamadroid)]    
        with open(path_roc_curve,'w') as file:
            json.dump(roc_curve,file)
...
def plot_roc_curve_zoom(model,malware_detector,roc_info = None):
    if roc_info is not None:
        mean_tpr,mean_fpr,std_tpr = roc_info
        ...
        if malware_detector == "Drebin":
            ...
            classifier = AdversarialDeepEnsembleMax() 
        tprs = []
        aucs = []
        #mean_fpr = np.linspace(0, 1, 100)
        ...
        if malware_detector == "MaMaDroid":
            y_train_res = np.array(model.y_train)
        elif malware_detector == "Drebin" or malware_detector == "Sec-SVM":
            y_train_res = model.y_train
        elif malware_detector == "AdversarialDeepEnsembleMax":
            X_train_res, valX, _ = utils_dnn.read_joblib(cfg.get('feature.drebin','dataX'))
            y_train_res, valy, _ = utils_dnn.read_joblib(cfg.get('feature.drebin','datay'))
            
        for train, test in cv.split(X_train_res, y_train_res):
            if malware_detector == "MaMaDroid" or malware_detector == "Drebin" or malware_detector == "Sec-SVM":
                classifier.fit(X_train_res[train], y_train_res[train])
            elif malware_detector == "AdversarialDeepEnsembleMax":
                classifier.train(trainX = X_train_res[train], trainy = y_train_res[train], valX = valX, valy = valy)        
            ...
            if malware_detector == "MaMaDroid":
                ...
                probas_ = classifier.test_rpst_new(X_train_res[test],y_train_res[test],'proba')
                probas_ = [item[1] for item in probas_]
    ...
    mean_tpr = np.array([val*100 for val in mean_tpr])
    mean_fpr = np.array([val*100 for val in mean_fpr])
    std_tpr = np.array([val*100 for val in std_tpr])
    
    #tprs_upper = np.minimum(mean_tpr + std_tpr, 1)
    #tprs_lower = np.maximum(mean_tpr - std_tpr, 0)
    
    tprs_upper = np.minimum(mean_tpr + std_tpr, 100)
    tprs_lower = np.maximum(mean_tpr - std_tpr, 0)
    ...
    if malware_detector == "Drebin":
        ...
        plt.plot(mean_fpr, mean_tpr, color='c',
             label=r'ADE-MA',
             lw=2, alpha=.8)        
        plt.fill_between(mean_fpr, tprs_lower, tprs_upper, color='c', alpha=.2) 
    ...
    return mean_tpr,mean_fpr,std_tpr
