...
def create_action_set(donors):
    ...
    for app in apps:        
        apks[app] = os.path.join(config['apks'],'accessible/normal',app)   
    ...
    organs = os.listdir(os.path.join(config['stored_components'],'organs'))
    action_set = dict()
    for organ in organs:
        action_set[organ] = os.path.join(config['stored_components'],'organs',organ)
...
def extract_an_action(feature,donor,donors,donor_path,donor_id):
    if  feature.split("::")[0] == "interesting_calls" or feature.split("::")[0] == "api_calls"or feature.split("::")[0] == "api_permissions":
        utils.perform_logging("feature: " + feature + " - donor "+ str(donor_id)+"("+donor+")" + " out of " + str(len(donors)) + " dononrs")
        print("feature: " + feature + " - donor "+ str(donor_id)+"("+donor+")" + " out of " + str(len(donors)) + " dononrs")

        print("Organ harvesting: start")
        organ = harvest_organ_from_donor(feature, donor_path, donor_id)  
        print("Organ harvesting: end")
        if organ != None:
            utils.perform_logging("organ harvesting corresponds " + feature + " from " + os.path.basename(donor_path) + " was successful" )
            print("organ harvesting corresponds " + feature + " from " + os.path.basename(donor_path) + " was successful" )                
            ...
            organ_path = os.path.join(config['stored_components'],'organs',"%(donor)s-%(feature)s_organ.p" %{"donor":donor,"feature": feature_name})
...
def harvest_organ_from_donor(feature, donor_path, donor_id):
    ...
    if os.path.exists(failure_test):
        utils.perform_logging('Previously failed to extract organ for feature {'+feature+'} from {'+donor_path+'}')
...
def extract(apk, j_feature, feature_type):
    ...
    try:
        out = subprocess.check_output(
            [config['java_sdk'] + 'java', '-jar', config['extractor'], j_feature,
