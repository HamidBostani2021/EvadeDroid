    ...
    private My_slice retrieve_3SAT(ArrayList<Unit> units_to_inject, ArrayList<Local> locals_to_inject,String opaque_path){
        ...
        try {
            ...
            for ( Local l : tmp_b.getLocals()){
            	...
            	System.out.println(l.getType().toString());
            ...
            for (Iterator<Unit> iter = tmp_b.getUnits().iterator(); iter.hasNext();){
                ...
                if(to_pru){
                	System.out.println(u.getBoxesPointingToThis().size());                   
                	...
                	/*Hamid
                		...
                		ArrayList<UnitBox> lu = (ArrayList<UnitBox>) u.getUnitBoxes();
                        for(UnitBox items: lu) {
                        	if(items.getUnit().getBoxesPointingToThis().size()>0) {
                        		injection = u;
                        	}            	
                        }
                		//u.addBoxPointingToThis(null);
                		//injection = u;
                	}
                	*/
                }
            ...
            System.out.println(tmp_b.getUnits());
            System.out.println(tmp_b.getUnits().getLast());
